...
import { GetAdmins, GetUsers } from '../providers';
...
export const useAdmin = (initialQuery: string = ''): IUseHookResponse => {
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const [data, setData] = useState<BankAccount[] | null>(null);
    const [message, setMessage] = useState<string>('');
    const [success, setSuccess] = useState<boolean>(false);
    ...
    const fetchUsers = async (query: string = '') => {
        ...
        try {
            ...
            setMessage('An error occurred while fetching Admin Profiles');
            setSuccess(false);
        } finally {
            setIsLoading(false);
        }
